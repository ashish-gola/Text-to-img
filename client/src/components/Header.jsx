...
const Header = () => {
  ...
  return (
    ...
    initial={{ opacity: 0.2, y: 100 }} transition={{ duration: 1 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
      ...
      <motion.button onClick={handleGenerateClick} className='sm:text-lg text-white bg-black w-auto mt-8 px-12 py-2.5 flex items-center gap-2 rounded-full'
      whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ default: { duration: 0.5 }, opacity: { delay: 0.8, duration: 1 } }}>
        Generate Images
        <img className='h-6' src={assets.star_icon} alt="star icon" />
        </motion.button>


        <motion.div className='flex items-center justify-center gap-3 mt-16 flex-wrap'
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 1 }}>
            {Array(6).fill('').map((item, index) => (
                <motion.img className='rounded hover:scale-105 transition-transform duration-300 cursor-pointer max-sm:w-10'
        ...
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 0.8 }}>Generated Images from imagify</motion.p>
